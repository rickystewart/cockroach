...
func makeRoachprodStressCmd(runE func(cmd *cobra.Command, args []string) error) *cobra.Command {
	roachprodStressCmd := &cobra.Command{
		Use:     "roachprod-stress pkg --cluster CLUSTER",
		Short:   "stress the given test on the given roachprod cluster",
		Long:    "stress the given test on the given roachprod cluster.",
		Example: "dev roachprod-stress ./pkg/cmd/dev --cluster my_cluster --stress-args '-arg1 -arg2'",
...
func (d *dev) roachprodStress(cmd *cobra.Command, commandLine []string) error {
	...
	args, buildTargets, err := d.getBasicBuildArgs(ctx, []string{"//pkg/cmd/roachprod", "//pkg/cmd/roachprod-stress"})
	if err != nil {
		return err
	}
	...
	roachprodStressArgs := []string{cluster, fmt.Sprintf("./%s", pkg), "-testbin", filepath.Join(workspace, "artifacts", testTargetBasename), "-stressbin", filepath.Join(workspace, "artifacts", "stress"), "-roachprodbin", filepath.Join(workspace, "bin", "roachprod"), "-libdir", filepath.Join(workspace, "artifacts", "libgeos", "lib")}
